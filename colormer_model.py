...
class colormer(object):
    """Implementation of colormer."""
    ...
    def save_model(self, epoch, stats, first=False):
        ...
        if self.p.ckpt_overwrite:
            fname_unet = '{}/colormer-{}.pt'.format(self.ckpt_dir, self.p.dataset_name)
        else:
            valid_loss = stats['valid_loss'][epoch]
            fname_unet = '{}/colormer-epoch{}-{:>1.5f}.pt'.format(self.ckpt_dir, epoch + 1, valid_loss)
        print('Saving checkpoint to: {}\n'.format(fname_unet))
        torch.save(self.model.state_dict(), fname_unet)

        # Save stats to JSON
        fname_dict = '{}/colormer-stats.json'.format(self.ckpt_dir)
    ...
    def eval(self, valid_loader):
        with torch.no_grad():
            ...
            for batch_idx, (source, target,haze_name) in enumerate(valid_loader):
                ...
                # colored
                source_colored = self.model(source)

                # Update loss
                loss = self.loss(source_colored, target)
                loss_meter.update(loss.item())

                # Compute PSRN
                for i in range(source_colored.shape[0]):
                    # import pdb;pdb.set_trace()
                    source_colored = source_colored.cpu()
                    target = target.cpu()
                    psnr_meter.update(psnr(source_colored[i], target[i]).item())
